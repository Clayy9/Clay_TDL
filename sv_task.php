...
if ($act == "set_done") {
    ...
    $sql = "SELECT tb_users.*, tb_pets.*, tb_pet_phases.* , (SELECT COUNT(*) FROM tb_tasks WHERE user_id='$user_id' AND status_id=2) AS task_completed FROM tb_users LEFT JOIN tb_pet_phases ON tb_users.pet_phases_id = tb_pet_phases.id LEFT JOIN tb_pets ON tb_users.pet_id = tb_pets.id WHERE tb_users.id='$user_id' && xp>=min_xp && xp<=max_xp";
    $query = mysqli_query($conn, $sql);
    ...
    ?>
                            ...
                            <div class="pet_display">
                                <img class="pet_display_img" id="pet_display_img" src="./assets/images/pet/<?php echo $resultUpdatePET['phase_img']; ?>" />
                            </div>
